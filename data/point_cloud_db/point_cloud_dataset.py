...
import numpy as np
from utils.tensor_utils import to_numpy, to_tensor
...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.hparams.OVERFIT_singel_pair is not None and self.hparams.OVERFIT_singel_pair is not False:
        ...
        min_num_points = X['pos'].shape[0] if X['pos'].shape[0] < Y['pos'].shape[0] else Y['pos'].shape[0]
        ...
        if min_num_points > self.hparams.num_points:
            ...
            if(self.gt_map is None):
                pass
            elif(isinstance(self.gt_map,np.ndarray) and np.allclose(self.gt_map[index],np.arange(self.gt_map[index].shape[0]))):
        ...
        if(self.gt_map is not None):
            data["gt_map"] = gt_map
        if(to_tensor(gt_map).allclose(torch.arange(gt_map.shape[0])) and Y["pos"].shape[0] < gt_map.shape[0]):
    ...
    def extract_soft_labels_per_pair(gt_map,target_pc,replace_on_cpu=False):
        ...
        for each_ratio in ratio_list:
            val = PointCloudDataset.make_soft_label(gt_map, target_pc, ratio=each_ratio) #torch.Size([1024, 1024])
            if(replace_on_cpu):
                val = val.cpu()
    ...
    def compute_or_load_soft_labels(indices_pairs,verts,gts,data_path):
        ...
        soft_labels = {}
        for idx,(source_idx,target_idx) in enumerate(tqdm(indices_pairs,desc='compute_soft_labels')):
            input1 = torch.tensor(verts[source_idx])
            input2 = torch.tensor(verts[target_idx])
            gt_map = gts[idx]
            ratio_list,soft_labels = PointCloudDataset.extract_soft_labels_per_pair(source_idx,target_idx,gt_map,input1,input2)
